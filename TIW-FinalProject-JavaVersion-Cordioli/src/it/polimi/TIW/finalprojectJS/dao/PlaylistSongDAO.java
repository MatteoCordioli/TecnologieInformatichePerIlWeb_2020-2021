...
public class PlaylistSongDAO {
	...
	public void updateSongOrder(int idSong, int order, int idPlaylist) throws SQLException {
		String query = "UPDATE playlistsong SET customOrder = ? WHERE idSong = ? AND idPlaylist=?";
		PreparedStatement pstatement = null;
		try {
			pstatement = con.prepareStatement(query);
			pstatement.setInt(1, order);
			pstatement.setInt(2, idSong);
			pstatement.setInt(3, idPlaylist);
			pstatement.executeUpdate();
		} catch (SQLException e) {
			throw new SQLException(e);
		} finally {
